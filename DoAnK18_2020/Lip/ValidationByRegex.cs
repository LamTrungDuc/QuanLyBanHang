...
namespace DoAnK18_2020.Lip
{
    class ValidationByRegex
    {
        #region Methods using Regular Expression to validate data
        /// <summary>
        /// Phương thức dùng để kiểm tra dữ liệu theo điều kiện yêu cầu. Kết quả trả về True nếu hợp lệ
        /// </summary>
        /// <param name="str">Chuỗi cần kiểm tra</param>
        /// <param name="patternForRegex">Mẫu được xem là điều kiện dùng cho kiểm tra dữ liệu</param>
        /// <returns></returns>
        private static bool checkRegex(string str, string patternForRegex)
        {
            Regex rg = new Regex(patternForRegex);
        ...
        /// <returns></returns>
        public static bool checkPasswordRegex(string password, out string notify)
        {
            notify = string.Empty;
        ...
        /// <param name="str"></param>
        /// <returns></returns>
        public static bool checkUsernameRegex(string str)
        {
            return checkRegex(str, @"^(?=[a-zA-Z0-9._]{8,16}$)(?!.*[_.]{2})[^_.].*[^_.]$");
        }
        public static bool checkDate(string str)
        {
            Regex rg = new Regex(@"^(0[1-9]|[12][0-9]|3[01])"   //Ngày
                               + @"[-](0[1-9]|1[012])"          //Tháng
                               + @"[-](19|20|21)\d\d$");        //Năm (1900 < year < 2200)
            bool flag = false;
            if (rg.IsMatch(str))
            {
                flag = true;
                ...
                switch (arr[1])
                {
                    //Tháng 4, 6, 9, 11 có 30 ngày => Nếu 31 thì flag = false
                    case "04":
                    case "06":
                    case "09":
                    case "11":
        ...
        /// <summary>
        /// Check Phone Number
        /// 3 số đầu phải đúng vs đầu số của các nhà mạng
        /// </summary>
        /// <param name="str"></param>
        /// <returns></returns>
        ...
        {
            //var mail = new System.Net.Mail.MailAddress(email); (Cách khác)
